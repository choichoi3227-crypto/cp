...
function renderOps({ usage = [], invoices = [], tickets = [] }) {
  usagePanel.innerHTML = usage.map((u) => `<p>${u.productId} · ${u.metric}: ${u.quantity.toLocaleString('ko-KR')} ${u.unit}</p>`).join('') || '<p>기록된 사용량 없음</p>';
  invoicePanel.innerHTML = invoices.map((i) => `<p>${i.id.slice(0, 8)} · $${Number(i.amount || 0).toFixed(2)} · ${i.status}</p>`).join('') || '<p>청구서 없음</p>';
  ticketPanel.innerHTML = tickets.map((t) => `<p>${t.subject} · ${t.status} · ${t.priority}</p>`).join('') || '<p>지원 티켓 없음</p>';
}
...
ticketForm.addEventListener('submit', async (event) => {
  event.preventDefault();
  const body = Object.fromEntries(new FormData(ticketForm));
  try {
    const data = await api('/support/tickets', { method: 'POST', body: JSON.stringify(body) });
    renderOps({ tickets: data.tickets });
    ticketForm.reset();
  } catch (error) { sessionPanel.textContent = error.message; }
});
...
checkoutBtn.addEventListener('click', async () => {
  try {
    ...
    billingPanel.innerHTML = data.paypal ? `청구서 ${data.invoice.id}: $${data.invoice.amount.toFixed(2)} · <a class="text-cyan-200 underline" href="${data.paypal.approveUrl}" target="_blank" rel="noreferrer">PayPal 승인</a>` : `관리자 권한으로 결제가 면제되었습니다. 청구서 ${data.invoice.id}`;
    const invoices = await api('/billing/invoices');
    renderOps({ invoices: invoices.invoices });
