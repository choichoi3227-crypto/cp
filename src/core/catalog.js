export const PRODUCTS = {
  cloudpressdb: {
    ...
    defaults: { replicas: 3, consistency: 'quorum', capacity: 'unlimited-logical', engine: 'cpdb-native' },
  },
  cp3: {
    type: 'cp3',
    name: 'CP3 Storage',
    summary: 'R2/S3를 사용하지 않는 자체 스토리지 패브릭',
    defaults: { includedGb: 50, overageKrwPerGb: 1900, erasureCoding: '10+4', engine: 'cp3-native' },
  },
  wordpress: {
    type: 'wordpress',
    name: 'WordPress Hosting',
    summary: 'PHP-WASM 기반 고성능 워드프레스 호스팅. DB/스토리지는 별도 상품으로 완전 분리',
    defaults: { runtime: 'php-wasm', db: 'separate-cloudpressdb', storage: 'separate-cp3', p95LatencyMsTarget: 80 },
  },
  static: {
    type: 'static',
    name: 'Static Site',
    summary: '직접 파일 작성과 즉시 배포를 지원하는 정적 사이트',
    defaults: { build: 'none', cache: 'edge-hot', editor: 'browser-files' },
  },
  php: {
    type: 'php',
    name: 'PHP Site',
    summary: 'PHP-WASM 런타임 기반 범용 PHP 사이트',
    defaults: { runtime: 'php-wasm', autoscale: true, isolation: 'per-project-sandbox' },
  },
  dns: {
    type: 'dns',
    name: 'CloudPress DNS',
  ...
  observability: {
    ...
    defaults: { metricsRetentionDays: 30, logsRetentionDays: 14, slo: '99.99%' },
  },
};
