...
export function estimateMonthlyCost(project, env = {}) {
  if (project.type === 'cp3') {
    ...
    return { currency: 'KRW', model: 'base_plus_additional', traffic: 'unlimited', trafficMonthly: 0, baseMonthly: 0, includedGb: 50, billableGb, additionalMonthly: billableGb * 1900, monthly: billableGb * 1900 };
  }
  if (project.type === 'dns') {
    const dns = estimateDnsMonthlyCost({ zones: project.config?.zones || 1, monthlyQueries: project.config?.monthlyQueries || 0 }, env);
    return { ...dns, model: 'base_plus_additional', traffic: 'unlimited', trafficMonthly: 0, baseMonthly: 0, additionalMonthly: dns.monthly };
  }
  ...
  return { currency: 'KRW', model: 'base_plus_additional', traffic: 'unlimited', trafficMonthly: 0, baseMonthly: pricing.baseMonthlyKrw, includedUsage: pricing.includedUsage, additionalUnit: pricing.unit, additionalUnits, additionalUnitKrw: pricing.additionalKrw, additionalMonthly, monthly: pricing.baseMonthlyKrw + additionalMonthly };
}

export function createInvoice({ owner, items }) {
  const subtotalKrw = items.reduce((sum, item) => sum + Number(item.billing?.monthly || 0), 0);
  return { id: crypto.randomUUID(), owner, currency: 'USD', krwTotal: subtotalKrw, amount: Number((subtotalKrw / 1350).toFixed(2)), description: 'CloudPress monthly base + additional usage charges; traffic is unlimited and not billed', items, createdAt: new Date().toISOString() };
}
