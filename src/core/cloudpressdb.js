...
export class CloudPressDBEngine {
  ...
  async createCheckout(user) {
    ...
    if (user.role === 'admin') {
      const waived = { ...invoice, amount: 0, krwTotal: 0, status: 'waived_admin', paymentRequired: false, reason: 'admin users can use CloudPress without payment' };
      await this.store.put(`invoice:${invoice.id}`, waived);
      await this.store.put(`owner-invoice:${user.email}:${invoice.id}`, invoice.id);
      await this.audit('billing.checkout.waive', user.email, { invoiceId: invoice.id, reason: 'admin' });
      return { invoice: waived, paypal: null };
    }
  ...
  async architecture() {
    return {
      ...
      guarantees: ['unlimited traffic with no traffic billing', 'admin users can use services without payment', 'isolated project topology', 'separate DB/storage products for WordPress', 'audit-first mutations', 'zero R2/S3 dependency in CP3 design'],
    };
  }
