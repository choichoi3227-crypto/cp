...
import { createInvoice } from '../src/core/billing.js';
...
test('hosting plans use unlimited traffic and invoices do not bill traffic', () => {
  const plan = createProvisioningPlan({ owner: 'owner@example.com', name: 'wp', type: 'wordpress' });
  assert.equal(plan.trafficPolicy, 'unlimited');
  assert.equal(plan.billing.traffic, 'unlimited');
  assert.equal(plan.billing.trafficMonthly, 0);
  const invoice = createInvoice({ owner: 'owner@example.com', items: [plan] });
  assert.match(invoice.description, /traffic is unlimited/);
});
